...
│           │   │   │   ├── CasAuthenticationController.java
...
| `.env.example` | Documents every required environment variable for local dev setup. Safe to commit — contains no real secrets. Includes `APP_BASE_URL` (used to build the CAS callback URL); no longer includes the old `SUPPORT_EMAIL`/`APP_PASSWORD` mail credentials. |
...
| `pom.xml` | Maven project descriptor. Declares all dependencies (Spring Boot, Spring Security, Spring Data JPA, PostgreSQL driver, JWT, Jakarta Validation, Lombok, `cas-client-core` for CAS SSO), the Java version (21), and the build plugin that packages the fat JAR. (The old `spring-boot-starter-mail` dependency was removed with the local email-verification flow.) |
...
| File | Purpose |
|---|---|
| `ApplicationConfiguration.java` | Defines the `UserDetailsService` bean (loads users from the DB by email) used by `JwtAuthenticationFilter` on every authenticated request. As of the CAS migration, this is all it defines — there's no password-based `AuthenticationManager`/`AuthenticationProvider`/`PasswordEncoder` anymore, since nothing compares passwords; CAS is the only login path and `JwtAuthenticationFilter` builds the `Authentication` object directly from a validated JWT. |
...
| `CasAuthenticationController.java` | The entire auth flow, replacing the old password-based `AuthenticationController`. Routes: `GET /auth/cas/login` (redirects the browser to Western's CAS login page), `GET /auth/cas/callback` (CAS redirects back here with a `?ticket=`; validates it via `Cas30ServiceTicketValidator`, delegates to `CasAuthenticationService` to find/create the user, mints a JWT via `JWTService`, redirects to the frontend's `/sso-callback` with the token or an error code). |
...
| `UserController.java` | Handles the authenticated user's account. Routes: `GET /users/me` (current user). (The old `PATCH /users/change-password` route was removed along with local passwords.) |
...
*(The old register/login/verify/forgot-password/reset-password/change-password DTOs were deleted with the CAS migration — there's no local password flow left to validate input for.)*
...
| File | Purpose |
|---|---|
| `User.java` | Core user entity. Fields: `id`, `email` (unique — matched against CAS's `mail` attribute), `password` (unused since the CAS migration — kept NOT NULL in the schema and filled with a random unusable value for CAS-provisioned users, since nothing compares it anymore), `role` (enum: STUDENT / RESEARCHER, set once at creation from CAS's `uwoRole` and never overwritten), `firstName`, `lastName`, `departmentNumber` (Western org-unit code from CAS, nullable), `verificationCode`, `verificationExpired`, `enabled` (now always `true` for CAS users — CAS already verified identity). Implements `UserDetails` for Spring Security. |
...
| File | Purpose |
|---|---|
| `CasAuthenticationService.java` | Turns a validated CAS `Assertion` into a local `User`: looks up by the `mail` attribute, or provisions a new row on first login (name/department refreshed from CAS on every login; role set only once, at creation). `mapRole()` maps the CAS `uwoRole` attribute to `STUDENT`/`RESEARCHER` — **currently incomplete**, see the `KNOWN ISSUE` comment in the file; real logins return a multi-valued list of short codes (e.g. `[UGS, STF]`) that the current mapping doesn't handle, pending WTS confirming the full code legend. |
| `JWTService.java` | JWT operations: `generateToken(User)` creates a signed JWT with email as subject and a configurable expiry. `extractUsername(token)` and `isTokenValid(token, user)` are used by the auth filter. Uses the `JWT_SECRET` env var as the signing key. Unchanged by the CAS migration — `CasAuthenticationController` calls the exact same method the old password login used to. |
...
| File | Purpose |
|---|---|
| `application.properties` | Base Spring Boot configuration loaded in all environments. All sensitive values are read from environment variables (`${VAR_NAME}`). Configures: datasource, JPA dialect, JWT, CORS allowed origins, frontend URL. Also imports a local `.env` file for dev convenience via `spring.config.import`. (SMTP/mail properties were removed along with the local email-verification flow.) |
| `application-prod.properties` | Production-only overrides, activated by `--spring.profiles.active=prod`. Sets `server.servlet.context-path=/api` (so Nginx can cleanly proxy `/api/` traffic), explicitly sets `ddl-auto=validate` (no automatic DB migration in production), and configures Western's CAS SSO: `cas.server.login-url`, `cas.server.url-prefix` (used to build the `p3/serviceValidate` call), and `cas.service.url` (built from the `APP_BASE_URL` env var). CAS replaced the originally-planned OIDC integration — WTS confirmed CAS is the only supported protocol for in-house apps. |
...
| File | Purpose |
|---|---|
| `.env` | Local development environment variables: `VITE_API_URL` (points the Axios client at the Spring Boot instance) and `VITE_CAS_LOGOUT_URL` (Western's CAS logout endpoint — currently unused in code, kept for a possible future proper CAS single-logout; see `DashboardSidebar.tsx`). |
| `.env.example` | Documents required env vars. Notes that production should use `VITE_API_URL=https://vrmm.eng.uwo.ca/api` to match the Nginx proxy and Spring Boot context path. `VITE_CAS_LOGOUT_URL` is the same value (`https://ssocas.uwo.ca/cas/logout`) in every environment since it's Western's endpoint, not ours. |
...
| `App.tsx` | Root application component. Sets up global providers (`QueryClientProvider`, `AuthProvider`, `TooltipProvider`, `Sonner`), `BrowserRouter`, and defines the complete route tree — public routes under `PublicLayout`, auth routes standalone (just `/login` and `/sso-callback` since the CAS migration — no more `/register`, `/forgot-password`, `/reset-password`, `/email-verification`), and dashboard routes guarded by `ProtectedRoute` with role-based access. |
...
| `PublicNavbar.tsx` | Navigation bar for public pages. Shows the VRMM logo, nav links (Browse, About), and a single "Sign In" button (linking to `/login`) — the separate Register button was removed with self-registration. |
...
| `DashboardSidebar.tsx` | Left navigation panel for the authenticated dashboard. Renders role-aware nav items — students see Browse and My Applications; researchers see My Postings and Create Posting. Both roles see Profile and Settings. Uses `NavLink` for active-state highlighting. Also owns the logout button (`handleLogout`) — clears the local JWT and returns to the landing page; a CAS single-logout redirect was tried and reverted (see the comment in the file) after `ssocas.uwo.ca` rejected the service URL. |
...
| File | Purpose |
|---|---|
| `AuthPages.tsx` | Shared layout component (`AuthCard`) used by both auth pages. Provides the centered card container, title, subtitle text, and a footer slot. Keeps auth page structure consistent. |
| `Login.tsx` | No longer a form — a single "Sign in with Western" button. Clicking it does a full-page navigation (not an Axios call) to the backend's `GET /auth/cas/login`, which starts the CAS redirect chain. See `SsoCallback.tsx` for where that chain ends. |
| `SsoCallback.tsx` | Landing point for the CAS round trip. The backend's last redirect (from `CasAuthenticationController.callback()`) lands here with either `?token=<jwt>` (success — calls `AuthContext.login()` and navigates to the dashboard) or `?error=<code>` (failure — shows a message, e.g. for an unrecognized CAS role). |

*(The old `Register.tsx`, `EmailVerification.tsx`, `ForgotPassword.tsx`, and `ResetPassword.tsx` were deleted with the CAS migration — self-registration and local password flows no longer exist.)*
...
| `SettingsPage.tsx` | Account settings page. Just an Appearance section noting the theme toggle location — the change-password form was removed with the CAS migration (no local passwords left to change). |
...
| File | Purpose |
|---|---|
| `Landing.tsx` | The homepage (`/`). Three sections: hero (headline, CTA buttons), feature grid (6 cards explaining platform capabilities), how-it-works (3-step visual), and a CTA banner linking to `/login` ("Sign In with Western"). |
